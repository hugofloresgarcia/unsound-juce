      ...
      panCoordLabel("coord", "0.50, 0.50")
{
    ...
    for (int i = 0; i < numVariations; ++i)
    {
        ...
        variation->allocate_buffer(sampleRate, 10.0); // 10 second max duration
    ...
    parameterKnobs.addKnob({
        ...
        "x",
        [this](double value) {
            looperEngine.getTrack(trackIndex).readHead.set_speed(static_cast<float>(value));
        },
    ...
    parameterKnobs.addKnob({
        ...
        [this](double value) {
            ...
            if (sampleRate > 0.0)
            {
                ...
                track.writeHead.set_wrap_pos(wrapPos);
    ...
    // Initialize duration to 5.0 seconds (default value)
    {
        ...
        double sampleRateInit = trackInit.writeHead.get_sample_rate();
        ...
        if (sampleRateInit > 0.0)
        {
            ...
            trackInit.writeHead.set_wrap_pos(wrapPos);
        }
    ...
    levelControl.onLevelChange = [this](double value) {
        looperEngine.getTrack(trackIndex).readHead.set_level_db(static_cast<float>(value));
    };
    ...
    outputSelector.onChannelChange = [this](int channel) {
        looperEngine.getTrack(trackIndex).readHead.set_output_channel(channel);
    };
...
void LooperTrack::paint(juce::Graphics& g)
{
    ...
    else if (track.isPlaying.load() && track.tapeLoop.m_has_recorded.load())
    {
...
void LooperTrack::playButtonClicked(bool shouldPlay)
{
    ...
    if (shouldPlay)
    {
        ...
        if (track.writeHead.get_record_enable() && !track.tapeLoop.m_has_recorded.load())
        {
            const juce::ScopedLock sl(track.tapeLoop.m_lock);
            track.tapeLoop.clear_buffer();
...
{
    auto& track = looperEngine.getTrack(trackIndex);
    track.readHead.set_muted(muted);
}
...
void LooperTrack::resetButtonClicked()
{
    ...
    if (sampleRate > 0.0)
    {
        track.writeHead.set_wrap_pos(static_cast<size_t>(5.0 * sampleRate));
    }
    
    levelControl.setLevelValue(0.0, juce::dontSendNotification);
    track.readHead.set_level_db(0.0f);
    
    // Unmute
    track.readHead.set_muted(false);
    ...
    track.readHead.set_output_channel(-1);
...
void LooperTrack::setPlaybackSpeed(float speed)
{
    ...
    looperEngine.getTrack(trackIndex).readHead.set_speed(speed);
}
...
void LooperTrack::timerCallback()
{
    ...
    float currentPos = track.readHead.get_pos();
...
void LooperTrack::applyVariationsFromFiles(const juce::Array<juce::File>& outputFiles)
{
    ...
    if (numReceived != numVariations)
    {
        ...
        {
            auto variation = std::make_unique<TapeLoop>();
            variation->allocate_buffer(sampleRate, 10.0);
            variations.push_back(std::move(variation));
        }
    }
...
void LooperTrack::switchToVariation(int variationIndex)
{
    ...
    // Copy variation buffer to active track buffer
    {
        ...
        const juce::ScopedLock slTrack(track.tapeLoop.m_lock);
        
        auto& varBuffer = variation->get_buffer();
        auto& trackBuffer = track.tapeLoop.get_buffer();
        
        if (varBuffer.empty() || trackBuffer.empty())
            return;
        
        size_t copyLength = juce::jmin(varBuffer.size(), trackBuffer.size(), variation->m_recorded_length.load());
        ...
        track.tapeLoop.m_recorded_length.store(copyLength);
        track.tapeLoop.m_has_recorded.store(true);
        
        // Update wrapPos
        track.writeHead.set_wrap_pos(copyLength);
        track.writeHead.set_pos(copyLength);
    }
    
    // Reset read head to start
    track.readHead.reset();
    track.readHead.set_pos(0.0f);
