...
void DualWaveformDisplay::drawWaveform(juce::Graphics& g, juce::Rectangle<int> area, TapeLoop& tapeLoop, LooperReadHead& readHead, LooperWriteHead& writeHead, bool isRecordBuffer)
{
    ...
    if (writeHead.getRecordEnable() && isRecordBuffer)
    {
    ...
    if (displayLength == 0 && !(writeHead.getRecordEnable() && isRecordBuffer))
    {
    ...
    if (isRecordBuffer)
    {
        waveformColor = writeHead.getRecordEnable() ? juce::Colour(0xfff04e36) : juce::Colour(0xff1eb19d);
    }
    else
    {
